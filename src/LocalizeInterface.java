 ...
 *
 * $Id: LocalizeInterface.java 133 2011-05-04 00:07:52Z corot $
 *
 */
package javaclient3;
        ...
        try {
            switch (header.getSubtype ()) {
                case PLAYER_LOCALIZE_DATA_HYPOTHS: {
                    ...
                    // Read pending_count, pending_time, hypoths_count; skip array_count
                    is.readFully (buffer, 0, 4 + 8 + 8);
                    ...
                    buffer = new byte[hypothsCount * (24 + 48 + 8)];

                    // Read the hypotheses
                    is.readFully (buffer, 0, hypothsCount * (24 + 48 + 8));
        ...
        try {
            switch (header.getSubtype ()) {
                ...
                case PLAYER_LOCALIZE_REQ_GET_PARTICLES:{
                    ...
                    byte[] buffer = new byte[24 + 8 + 8];

                    // Read mean, variance, particles_count; skip array_count
                    is.readFully (buffer, 0, 24 + 8 + 8);
